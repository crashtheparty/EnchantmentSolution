...
public class EquipListener implements Listener{
	...
	public void onInventoryClickArmor(final InventoryClickEvent e){
		...
		if(!shift && newItemSlotType != null && e.getRawSlot() != newItemSlotType.getSlot()){
			return;
		}
		if(shift){
			...
			if(newItemSlotType != null){
				...
				if(e.getRawSlot() == newItemSlotType.getSlot()){
					equipping = false;
				}
	...
	public void onPlayerInteractArmor(PlayerInteractEvent e){
		...
		if(e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK){
			...
			if(newItemSlotType != null){
				if(newItemSlotType.equals(ItemSlotType.HELMET) && isAirOrNull(e.getPlayer().getInventory().getHelmet()) || newItemSlotType.equals(ItemSlotType.CHESTPLATE) && isAirOrNull(e.getPlayer().getInventory().getChestplate()) || newItemSlotType.equals(ItemSlotType.LEGGINGS) && isAirOrNull(e.getPlayer().getInventory().getLeggings()) || newItemSlotType.equals(ItemSlotType.BOOTS) && isAirOrNull(e.getPlayer().getInventory().getBoots())){
	...
	public void onPlayerJoinArmor(PlayerJoinEvent event) {
		...
		for(ItemStack item : player.getInventory().getArmorContents()) {
			...
			if(type != null){
				Bukkit.getServer().getPluginManager().callEvent(new ArmorEquipEvent(player, EquipMethod.JOIN, type, null, item));
			}
		}
	}
	...
	public void onPlayerDeathArmor(PlayerDeathEvent e){
		...
		for(ItemStack i : p.getInventory().getArmorContents()){
			if(!isAirOrNull(i)){
				Bukkit.getServer().getPluginManager().callEvent(new ArmorEquipEvent(p, EquipMethod.DEATH, ItemSlotType.matchArmorType(i), i, null));
			}
		}
	}
	...
	public void onPlayerRespawnArmor(PlayerRespawnEvent e){
		...
		for(ItemStack i : p.getInventory().getArmorContents()){
			if(!isAirOrNull(i)){
				Bukkit.getServer().getPluginManager().callEvent(new ArmorEquipEvent(p, EquipMethod.DEATH, ItemSlotType.matchArmorType(i), null, i));
			}
		}
	}
	...
	public void onInventoryClickHandEquip(InventoryClickEvent e) {
		...
		if (dropping) {
			if (e.getSlotType().equals(InventoryType.SlotType.QUICKBAR) && e.getSlot() == holdslot
				...
				ItemEquipEvent event = new ItemEquipEvent((Player) e.getWhoClicked(), ItemEquipEvent.HandMethod.DROP, ItemSlotType.MAIN_HAND, 
				...
				ItemEquipEvent event = new ItemEquipEvent((Player) e.getWhoClicked(), ItemEquipEvent.HandMethod.DROP, ItemSlotType.OFF_HAND, 
						e.getCurrentItem(), null);
			...
			if (clicked - 36 == holdslot) {
				ItemEquipEvent event = new ItemEquipEvent((Player) e.getWhoClicked(), ItemEquipEvent.HandMethod.HELD_SWAP, ItemSlotType.MAIN_HAND, 
						e.getCurrentItem(), null);
				Bukkit.getServer().getPluginManager().callEvent(event);
			...
			if (numberkey) {
				...
				ItemEquipEvent event = new ItemEquipEvent((Player) e.getWhoClicked(), ItemEquipEvent.HandMethod.HOT_BAR, ItemSlotType.MAIN_HAND, 
						oldItem, newItem);
				...
				ItemEquipEvent event = new ItemEquipEvent((Player) e.getWhoClicked(), ItemEquipEvent.HandMethod.HOT_BAR, ItemSlotType.OFF_HAND, 
						oldItem, newItem);

...
public class FishingListener extends EnchantmentListener{
	...
	public void onPlayerFish(PlayerFishEvent event) {
		if(event.getState().equals(PlayerFishEvent.State.CAUGHT_FISH)) {
			...
			if(Enchantments.hasEnchantment(rod, DefaultEnchantments.ANGLER)) if(canRun(DefaultEnchantments.ANGLER, event)) {
			...
			if(Enchantments.hasEnchantment(rod, DefaultEnchantments.FRIED)) if(canRun(DefaultEnchantments.FRIED, event)) if (caught.getType().equals(Material.COD)) {
				caught.setType(Material.COOKED_COD);
				if(EnchantmentSolution.getPlugin().isJobsEnabled()) JobsUtils.sendFishAction(event);

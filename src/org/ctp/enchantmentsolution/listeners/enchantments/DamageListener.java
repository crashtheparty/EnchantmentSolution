...
public class DamageListener extends Enchantmentable {
	...
	private void sandVeil(EntityDamageByEntityEvent event) {
		...
		if(ea != null && event.getEntity() instanceof LivingEntity && damager instanceof LivingEntity) {
			...
			SandVeilMissEvent sandVeil = new SandVeilMissEvent((LivingEntity) event.getEntity(), (LivingEntity) damager, event.getDamage(), true, accuracy);
		...
		if(!event.isCancelled()) {
			if(damager instanceof Player && event.getEntity() instanceof LivingEntity) {
				...
				if(ItemUtils.hasEnchantment(item, RegisterEnchantments.SAND_VEIL)) {
					...
					SandVeilEvent sandVeil = new SandVeilEvent(entity, player, event.getDamage(), event.getDamage(), 160, accuracy);
					Bukkit.getPluginManager().callEvent(sandVeil);
					...
					if(!sandVeil.isCancelled()) {
						if(ea != null) {
							...
							EnchantmentSolution.addAccuracy(new EntityAccuracy(player, entity, sandVeil.getAccuracy(), sandVeil.getTicks()));
						}
					}
				}
			}
				...
				if(event.getDamager() instanceof Arrow) {
					...
					if(shooter instanceof HumanEntity) {
						...
						if(ItemUtils.hasEnchantment(item, RegisterEnchantments.STONE_THROW)) {
							...
							StoneThrowEvent stoneThrow = new StoneThrowEvent(entity, human, event.getDamage(), extraDamage);
							...
							if(!stoneThrow.isCancelled()) {
								...
								if(human instanceof Player && entity instanceof Phantom) {
									...
									if(phantom.getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue() == phantom.getHealth() && phantom.getHealth() <= extraDamage) {
										AdvancementUtils.awardCriteria((Player) human, ESAdvancement.JUST_DIE_ALREADY, "phantom");
									}
								}
								if(human instanceof Player && entity instanceof EnderDragon) {
									AdvancementUtils.awardCriteria((Player) human, ESAdvancement.UNDERKILL, "dragon");
								}
							}
						}
					}
		...
		if(!(entity instanceof WaterMob) && !(entity instanceof Drowned) && !(entity instanceof Guardian)) {
			...
			if(!drowned.isCancelled()) {
				if(drowned.willApplyEffect()) {
					EnchantmentSolution.addDrowned(new DrownedEntity((LivingEntity) drowned.getEntity(), (HumanEntity) drowned.getDamager(), drowned.getTicks()));
				}

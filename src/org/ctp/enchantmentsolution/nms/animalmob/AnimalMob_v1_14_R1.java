...
public class AnimalMob_v1_14_R1 extends AnimalMob {
	...
	public static AnimalMob createFromConfig(YamlConfig config, int i) {
		...
		try {
			((AnimalMob_v1_14_R1) mob).setCatType(Type.valueOf(config.getString("animals." + i + ".cat_type")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_14_R1) mob).setPandaMainGene(Gene.valueOf(config.getString("animals." + i + ".panda_main_gene")));
		} catch (Exception ex) {}
		try {

...
public class AnimalMob_v1_17_R1 extends AnimalMob {
	...
	private boolean isScreaming, isPlayingDead;
	private Axolotl.Variant axolotlVariant;

	public AnimalMob_v1_17_R1(Creature mob, ItemStack item) {
		super(mob, item);
		...
		if (mob instanceof GlowSquid) {
			GlowSquid glow = (GlowSquid) mob;
			setDarkTicksRemaining(glow.getDarkTicksRemaining());
		}
		if (mob instanceof Axolotl) {
			Axolotl axolotl = (Axolotl) mob;
			setAxolotlVariant(axolotl.getVariant());
			setPlayingDead(axolotl.isPlayingDead());
		}
		if (mob instanceof Goat) {
			Goat goat = (Goat) mob;
			setScreaming(goat.isScreaming());
		}
	}
	...
	public void editProperties(Entity e, boolean b1, boolean b2) {
		...
		try {
			...
			if (e instanceof GlowSquid) {
				GlowSquid glow = (GlowSquid) e;
				glow.setDarkTicksRemaining(getDarkTicksRemaining());
			}
			if (e instanceof Axolotl) {
				Axolotl axolotl = (Axolotl) e;
				axolotl.setVariant(getAxolotlVariant());
				axolotl.setPlayingDead(isPlayingDead());
			}
	...
	public void setConfig(Configurable configurable, String location, int i) {
		...
		config.set(location + i + ".axolotl_variant", getAxolotlVariant() != null ? getAxolotlVariant().name() : null);
		config.set(location + i + ".is_playing_dead", isPlayingDead());
		config.set(location + i + ".is_screaming", isScreaming());
		config.set(location + i + ".dark_ticks_remaining", getDarkTicksRemaining());
	}
	...
	public static AnimalMob createFromConfig(Configurable config, int i) {
		...
		((AnimalMob_v1_17_R1) mob).setPlayingDead(config.getBoolean("animals." + i + ".is_playing_dead"));
		((AnimalMob_v1_17_R1) mob).setScreaming(config.getBoolean("animals." + i + ".is_screaming"));
		((AnimalMob_v1_17_R1) mob).setDarkTicksRemaining(config.getInt("animals." + i + ".dark_ticks_remaining"));
		...
		try {
			((AnimalMob_v1_17_R1) mob).setCatType(Type.valueOf(config.getString("animals." + i + ".cat_type")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_17_R1) mob).setPandaMainGene(Gene.valueOf(config.getString("animals." + i + ".panda_main_gene")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_17_R1) mob).setPandaHiddenGene(Gene.valueOf(config.getString("animals." + i + ".panda_hidden_gene")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_17_R1) mob).setCollarColor(DyeColor.valueOf(config.getString("animals." + i + ".collar_color")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_17_R1) mob).setMooshroomType(MushroomCow.Variant.valueOf(config.getString("animals." + i + ".mooshroom_cow_variant")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_17_R1) mob).setFoxType(Fox.Type.valueOf(config.getString("animals." + i + ".fox_type")));
		} catch (Exception ex) {}
		try {
			((AnimalMob_v1_17_R1) mob).setAxolotlVariant(Axolotl.Variant.valueOf(config.getString("animals." + i + ".axolotl_variant")));
		} catch (Exception ex) {}
		try {

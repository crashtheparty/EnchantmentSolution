...
import org.ctp.enchantmentsolution.events.interact.FrostyEvent;
...
public class Frosty extends ShootEntityEffect {

	public Frosty() {
		super(RegisterEnchantments.FROSTY, EnchantmentMultipleType.HIGHEST, EnchantmentItemLocation.ATTACKED, EventPriority.HIGHEST, new EntityType[] { EntityType.SNOWBALL }, false, true, true, false, 1, "1", false, false, new InteractCondition[0]);
	}
	...
	public ShootEntityResult run(Player player, ItemStack[] items, PlayerInteractEvent event) {
		...
		FrostyEvent frosty = new FrostyEvent(player, item, takeItem, hasItem);
		...
		if (!frosty.isCancelled() && !frosty.willCancel()) {
			...
			if (itemType != null && (isAllowOffhand() || slot.getType() != ItemSlotType.OFF_HAND)) {
				...
				if (!frosty.takeSnowball()) projectile.setMetadata("no_pickup", new FixedMetadataValue(EnchantmentSolution.getPlugin(), true));
				DamageUtils.damageItem(player, item);
				player.getWorld().playSound(player.getLocation(), itemType.getSound(), 1, 1);

				ESPlayer esPlayer = EnchantmentSolution.getESPlayer(player);
				if (willSetCooldown()) esPlayer.setCooldown(RegisterEnchantments.FROSTY, frosty.getCooldownTicks());
				if (frosty.takeSnowball()) {

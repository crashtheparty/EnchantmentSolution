...
public class HealthPercentDamagedCondition implements DamageCondition {

	private double percent;
	private boolean less;

	public HealthPercentDamagedCondition(double percent, boolean less) {
	...
	public boolean metCondition(Entity damager, Entity damaged, EntityDamageByEntityEvent event) {
		if (damaged instanceof Damageable && damaged instanceof Attributable) {
			Damageable d = (Damageable) damaged;
			AttributeInstance instance = ((Attributable) damaged).getAttribute(Attribute.GENERIC_MAX_HEALTH);

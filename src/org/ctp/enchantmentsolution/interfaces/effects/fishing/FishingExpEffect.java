...
public abstract class FishingExpEffect extends FishingEffect {
	...
	public FishingExpEffect(EnchantmentWrapper enchantment, EnchantmentMultipleType type, EnchantmentItemLocation location, EventPriority priority,
	...
	boolean useExpPercent, boolean useExpPercentLevel, double expChance, boolean useExpChance, FishingCondition[] conditions) {
	...
	public FishingExpResult run(Player player, ItemStack[] items, PlayerFishEvent event) {
		...
		return new FishingExpResult(level, (int) exp);
	}
	...
	public class FishingExpResult extends EffectResult {

		private final int exp;

		public FishingExpResult(int level, int exp) {
			super(level);
			this.exp = exp;
		}

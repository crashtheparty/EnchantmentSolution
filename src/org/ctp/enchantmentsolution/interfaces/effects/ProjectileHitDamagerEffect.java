...
public abstract class ProjectileHitDamagerEffect extends ProjectileEffect {

	public ProjectileHitDamagerEffect(EnchantmentWrapper enchantment, EnchantmentMultipleType type, EnchantmentItemLocation location, EventPriority priority,
	...
	public boolean willRun(LivingEntity shooter, LivingEntity damaged, ProjectileHitEvent event) {
		for(ProjectileCondition condition: getConditions())
			if (!((ProjectileHitCondition) condition).metCondition(shooter, damaged, event)) return false;
		return true;
	}

...
public abstract class ModifyExperienceEffect extends EntityKillerEffect {
	...
	public ModifyExperienceEffect(EnchantmentWrapper enchantment, EnchantmentMultipleType type, EnchantmentItemLocation location, EventPriority priority,
	...
	boolean useExpPercent, boolean useExpPercentLevel, double expChance, boolean useExpChance, DeathCondition[] conditions) {
	...
	public ModifyExperienceResult run(Entity killer, Entity killed, ItemStack[] items, Collection<ItemStack> drops, EntityDeathEvent event) {
		...
		return new ModifyExperienceResult(level, (int) exp);
	}
	...
	public class ModifyExperienceResult extends EffectResult {

		private final int exp;

		public ModifyExperienceResult(int level, int exp) {
			super(level);
			this.exp = exp;
		}

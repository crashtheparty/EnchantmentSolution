...
public class Smeltery extends ChangeBlockDropEffect {

	public Smeltery() {
		super(RegisterEnchantments.SMELTERY, EnchantmentMultipleType.HIGHEST, EnchantmentItemLocation.ATTACKED, EventPriority.HIGH, new EnchantmentDrops("/resources/drops/smeltery.yml", false), new BlockCondition[0]);
	}
	...
	public ChangeBlockDropResult run(Player player, ItemStack[] items, BlockData brokenData, BlockDropItemEvent event) {
		...
		SmelteryEvent smeltery = new SmelteryEvent(event.getBlock(), brokenData, player, dropItems, result.getOldDrops(), interactions.isOverride(), interactions.getExp(level));
		Bukkit.getPluginManager().callEvent(smeltery);

		if (!smeltery.isCancelled()) {
			Location dropLoc = event.getBlock().getLocation();
			ExpNMS.dropExp(dropLoc, smeltery.getExp(), true, player);
			if (smeltery.willOverride()) event.setCancelled(true);
			List<Item> setItems = new ArrayList<Item>();
			for(ItemStack i: smeltery.getItems())

...
public class AbilityUtils {
	...
	public static ItemStack getGoldDiggerItems(ItemStack item,
		...
		int level = ItemUtils.getLevel(item,
				RegisterEnchantments.GOLD_DIGGER);
		int amount = 0;
		while(level > 0) {
	...
	public static void giveExperience(Player player, int amount) {
		...
		if(playerInv.getItemInMainHand() != null && ItemUtils.hasEnchantment(playerInv.getItemInMainHand(), Enchantment.MENDING)) {
			items.add(playerInv.getItemInMainHand());
		}
		if(playerInv.getItemInOffHand() != null && ItemUtils.hasEnchantment(playerInv.getItemInOffHand(), Enchantment.MENDING)) {
	...
	public static int getExhaustionCurse(Player player) {
		...
		for(ItemStack item : player.getInventory().getArmorContents()) {
			if(item != null && ItemUtils.hasEnchantment(item, RegisterEnchantments.CURSE_OF_EXHAUSTION)){
				exhaustionCurse += ItemUtils.getLevel(item, RegisterEnchantments.CURSE_OF_EXHAUSTION);
			}
		}
		ItemStack mainHand = player.getInventory().getItemInMainHand();
		if(mainHand != null && ItemUtils.hasEnchantment(mainHand, RegisterEnchantments.CURSE_OF_EXHAUSTION)){
			exhaustionCurse += ItemUtils.getLevel(mainHand, RegisterEnchantments.CURSE_OF_EXHAUSTION);
		}
		ItemStack offHand = player.getInventory().getItemInOffHand();
		if(offHand != null && ItemUtils.hasEnchantment(offHand, RegisterEnchantments.CURSE_OF_EXHAUSTION)){
			exhaustionCurse += ItemUtils.getLevel(offHand, RegisterEnchantments.CURSE_OF_EXHAUSTION);
		}

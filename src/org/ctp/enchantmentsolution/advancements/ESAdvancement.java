...
public enum ESAdvancement {
	...
	PRE_COMBAT_UPDATE(NOT_VERY_EFFECTIVE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/pre_combat_update"), 
			Arrays.asList(new ESLocalization(Language.US, "Pre-Combat Update", "Kill a player with a Quick Strike weapon")), "IRON_SWORD",
			Arrays.asList(new ESTrigger("combat_update", new ImpossibleTrigger())),
			new Rewards().setExperience(80), Frame.TASK),
	HEADHUNTER(PRE_COMBAT_UPDATE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/headhunter"), 
	...
	HIGH_METABOLISM(FEAR_THE_REAPER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/high_metabolism"), 
			Arrays.asList(new ESLocalization(Language.US, "High Metabolism", "Quadruple the amount of exhaustion experienced using Curse of Exhaustion")), 
	...
	ARMORED_EVOLUTION(TOO_CLOSE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/armored_evolution"), 
			Arrays.asList(new ESLocalization(Language.US, "Armored Evolution", "Upgrade your Elytra with armor using Armored")), 
	...
	GRAPHENE_ARMOR(CRUISING_ALTITUDE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/graphene_armor"), 
			Arrays.asList(new ESLocalization(Language.US, "Graphene Armor", "Gain 20 armor toughness using Toughness")), "DIAMOND_CHESTPLATE",
			Arrays.asList(new ESTrigger("toughness", new ImpossibleTrigger())), new Rewards().setExperience(200), Frame.GOAL),
	DANGER_DEFEATED(GRAPHENE_ARMOR, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/danger_defeated"), 

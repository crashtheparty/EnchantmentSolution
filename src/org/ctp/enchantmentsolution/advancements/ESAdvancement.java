...
import org.ctp.enchantmentsolution.external_api.hu.trigary.advancementcreator.Advancement.Frame;
import org.ctp.enchantmentsolution.external_api.hu.trigary.advancementcreator.Rewards;
import org.ctp.enchantmentsolution.external_api.hu.trigary.advancementcreator.shared.MaterialId;
import org.ctp.enchantmentsolution.external_api.hu.trigary.advancementcreator.trigger.ImpossibleTrigger;
...
public enum ESAdvancement {
	...
	ENCHANTMENT_SOLUTION(new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/enchantment_solution"), 
			...
			MaterialId.ENCHANTED_BOOK, Arrays.asList(), new Rewards(), Frame.TASK),
	FISH_STICKS(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/fish_sticks"), 
			Arrays.asList(new ESLocalization(Language.US, "Fish Sticks", "Cook a fish with Fried")), MaterialId.COOKED_COD,
	...
	FED_FOR_A_LIFETIME(FISH_STICKS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/fed_for_a_lifetime"), 
			Arrays.asList(new ESLocalization(Language.US, "Fed For a Lifetime", "Get more than one of each kind of fish with Angler")), MaterialId.COD,
	...
	NEMO_ENIM_COQUIT(FISH_STICKS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/nemo_enim_coquit"), 
			Arrays.asList(new ESLocalization(Language.US, "Nemo Enim Coquit", "Catch a tropical fish with Fried")), MaterialId.TROPICAL_FISH,
	...
	BREAKER_BREAKER(NEMO_ENIM_COQUIT, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/breaker_breaker"), 
			Arrays.asList(new ESLocalization(Language.US, "Breaker, Breaker", "Place blocks over a torch using Wand")), MaterialId.TORCH,
	...
	THORGY(BREAKER_BREAKER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/thorgy"), 
			Arrays.asList(new ESLocalization(Language.US, "Thorgy!", "Put a tamed wolf into your Irene's Lasso")), MaterialId.WOLF_SPAWN_EGG,
	...
	DID_YOU_REALLY_WAND_TO_DO_THAT(THORGY, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/did_you_really_wand_to_do_that"), 
			...
			MaterialId.CARROT_ON_A_STICK, Arrays.asList(new ESTrigger("break", new ImpossibleTrigger())), new Rewards().setExperience(100), Frame.GOAL),
	FREE_PETS(DID_YOU_REALLY_WAND_TO_DO_THAT, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/free_pets"), 
			Arrays.asList(new ESLocalization(Language.US, "Free Pets", "Grab every tameable animal in Irene's Lasso")), MaterialId.CARROT_ON_A_STICK,
	...
	SHARING_IS_CARING(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/sharing_is_caring"), 
			Arrays.asList(new ESLocalization(Language.US, "Sharing Is Caring", "Give your friend an item with Exp. Share")), MaterialId.EXPERIENCE_BOTTLE,
	...
	NOT_THAT_KIND(SHARING_IS_CARING, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/not_that_kind"), 
			Arrays.asList(new ESLocalization(Language.US, "Not That Kind", "Kill every breedable animal with Knock Up")), MaterialId.DIAMOND_SWORD,
			...
			new ESTrigger("llama", new ImpossibleTrigger()), new ESTrigger("mooshroom", new ImpossibleTrigger()),
			...
			new ESTrigger("wolf", new ImpossibleTrigger()), new ESTrigger("tu vrtle", new ImpossibleTrigger())),
	...
	SUPER_CHARGED(NOT_THAT_KIND, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/super_charged"), 
			Arrays.asList(new ESLocalization(Language.US, "Super Charged", "Create a charged creeper with a Shock Aspect attack")), MaterialId.DIAMOND_AXE,
	...
	NOT_VERY_EFFECTIVE(SUPER_CHARGED, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/not_very_effective"), 
			Arrays.asList(new ESLocalization(Language.US, "Not Very Effective...", "Kill a Drowned with a Brine sword")), MaterialId.NAUTILUS_SHELL,
	...
	HEADHUNTER(NOT_VERY_EFFECTIVE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/headhunter"), 
			Arrays.asList(new ESLocalization(Language.US, "Headhunter", "Steal a player's head")), MaterialId.PLAYER_HEAD,
	...
	DOUBLE_HEADER(HEADHUNTER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/double_header"), 
			Arrays.asList(new ESLocalization(Language.US, "Double Header", "Get two Wither Skeleton skulls from one kill")), MaterialId.WITHER_SKELETON_SKULL,
	...
	SUPER_EFFECTIVE(DOUBLE_HEADER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/super_effective"), 
			Arrays.asList(new ESLocalization(Language.US, "Super Effective!", "Kill a Minecraft boss with a Brine sword")), MaterialId.HEART_OF_THE_SEA,
	...
	MOTHERLOAD(SUPER_EFFECTIVE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/motherload"), 
			Arrays.asList(new ESLocalization(Language.US, "Motherload", "Kill the Ender Dragon using Exp. Share")), MaterialId.DRAGON_EGG,
			Arrays.asList(new ESTrigger("dragon", new ImpossibleTrigger())), 
			new Rewards().setExperience(1000), Frame.CHALLENGE),
	HEX_BAG(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/hex_bag"), 
			Arrays.asList(new ESLocalization(Language.US, "Hex Bag", "Drown a player using Drowned")), MaterialId.WITCH_SPAWN_EGG,
	...
	SEVEN_POINT_EIGHT(HEX_BAG, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/seven_point_eight"), 
			Arrays.asList(new ESLocalization(Language.US, "7.8/10", "Have 3 mobs drowning at the same time with Drowned")), MaterialId.WATER_BUCKET,
	...
	POSEIDONS_DAY_OFF(SEVEN_POINT_EIGHT, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/poseidons_day_off"), 
			...
			MaterialId.PRISMARINE_SHARD, Arrays.asList(new ESTrigger("day_off", new ImpossibleTrigger())), new Rewards().setExperience(50), Frame.TASK),
	FISHIER_BUSINESS(POSEIDONS_DAY_OFF, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/fishy_business"), 
			Arrays.asList(new ESLocalization(Language.US, "Fishier Business", "Collect all types of fish using Transmutation")), MaterialId.COD,
			Arrays.asList(new ESTrigger("cod", new ImpossibleTrigger()), new ESTrigger("salmon", new ImpossibleTrigger()), 
			new ESTrigger("clownfish", new ImpossibleTrigger()), new ESTrigger("pufferfish", new ImpossibleTrigger())), 
	...
	POSEIDON_REBORN(FISHIER_BUSINESS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/poseidon_reborn"), 
			Arrays.asList(new ESLocalization(Language.US, "Poseidon Reborn", "Use Transmutation to obtain a Trident")), MaterialId.TRIDENT,
	...
	CERBERUS(POSEIDON_REBORN, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/cerberus"), 
			Arrays.asList(new ESLocalization(Language.US, "Cerberus?", "Increase your reward while using a Transmutation trident")), MaterialId.SOUL_SAND,
	...
	BONEMEAL_PLUS(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/bonemeal_plus"), 
			Arrays.asList(new ESLocalization(Language.US, "Bonemeal Plus", "Use Flower Gift on a two-high flower")), MaterialId.BONE_MEAL,
	...
	FOURTY_NINERS(BONEMEAL_PLUS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/fourty_niners"), 
			Arrays.asList(new ESLocalization(Language.US, "49ers", "Get 81 nuggets from using Gold Digger")), MaterialId.GOLD_BLOCK,
	...
	CHICKEN_OR_THE_EGG(FOURTY_NINERS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/chicken_or_the_egg"), 
			Arrays.asList(new ESLocalization(Language.US, "Chicken or the Egg?", "Hit a chicken with an egg using Splatter Fest")), MaterialId.EGG,
	...
	EGGED_BY_MYSELF(CHICKEN_OR_THE_EGG, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/egged_by_myself"), 
			Arrays.asList(new ESLocalization(Language.US, "Egged By Myself", "Hit yourself with an egg launched from Splatter Fest")), MaterialId.EGG,
	...
	MISSED(EGGED_BY_MYSELF, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/missed"), 
			Arrays.asList(new ESLocalization(Language.US, "Missed", "Make 16 attacks miss with Sand Veil")), MaterialId.SAND,
	...
	JUST_AS_SWEET(MISSED, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/just_as_sweet"), 
			Arrays.asList(new ESLocalization(Language.US, "Just As Sweet", "Successfully get a Wither Rose using Flower Gift")), MaterialId.WITHER_ROSE,
	...
	LAAAGGGGGG(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/laaagggggg"), 
			Arrays.asList(new ESLocalization(Language.US, "Laaagggggg", "Lag the server with various particle effects")), MaterialId.FIREWORK_ROCKET,
	...
	EASY_OUT(LAAAGGGGGG, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/easy_out"), 
			Arrays.asList(new ESLocalization(Language.US, "Easy Out", "Turn off the campfire using Moisturize")), MaterialId.CAMPFIRE,
	...
	JUST_ADD_WATER(EASY_OUT, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/just_add_water"), 
			Arrays.asList(new ESLocalization(Language.US, "Just Add Water", "Add water to concrete powder using Moisturize")), MaterialId.WHITE_CONCRETE_POWDER,
	...
	REPAIRED(JUST_ADD_WATER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/repaired"), 
			Arrays.asList(new ESLocalization(Language.US, "Repaired", "Put a tamed wolf into your Irene's Lasso")), MaterialId.STONE_BRICKS,
	...
	KEPT_ON_HAND(REPAIRED, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/kept_on_hand"), 
			Arrays.asList(new ESLocalization(Language.US, "Kept On Hand", "Keep an item after death using Soulbound")), MaterialId.ENCHANTED_BOOK,
	...
	FEAR_THE_REAPER(KEPT_ON_HAND, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/fear_the_reaper"), 
			Arrays.asList(new ESLocalization(Language.US, "Fear the Reaper", "Steal another player's item using Soul Reaper")), MaterialId.DIAMOND_HOE,
	...
	READY_AFTER_DEATH(FEAR_THE_REAPER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/ready_after_death"), 
			...
			MaterialId.DIAMOND_CHESTPLATE, Arrays.asList(new ESTrigger("soulbound", new ImpossibleTrigger())), new Rewards().setExperience(400), Frame.GOAL),
	REAPED_THE_REAPER(READY_AFTER_DEATH, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/reaped_the_reaper"), 
			Arrays.asList(new ESLocalization(Language.US, "Reaped the Reaper", "Steal another player's Soul Reaper item with Soul Reaper")), MaterialId.DIAMOND_HOE,
	...
	COFFEE_BREAK(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/coffee_break"), 
			Arrays.asList(new ESLocalization(Language.US, "Coffee Break", "Put a No Rest helmet on when Phantoms can spawn")), MaterialId.SUGAR,
	...
	THIS_GIRL_IS_ON_FIRE(COFFEE_BREAK, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/this_girl_is_on_fire"), 
			Arrays.asList(new ESLocalization(Language.US, "This Girl is on Fire", "While walking on Magma-ized lava, catch fire")), MaterialId.LAVA_BUCKET,
	...
	FLAME_KEEPER(THIS_GIRL_IS_ON_FIRE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/flame_keeper"), 
			Arrays.asList(new ESLocalization(Language.US, "Flame Keeper", "Create magma in a world that shouldn't have lava")), MaterialId.FLINT_AND_STEEL,
			Arrays.asList(new ESTrigger("flame", new ImpossibleTrigger())), new Rewards().setExperience(40), Frame.TASK),
	THESE_BOOTS_WERE_MADE_FOR_WALKING(FLAME_KEEPER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/these_boots_were_made_for_walking"), 
			Arrays.asList(new ESLocalization(Language.US, "These Boots Were Made for Walking", "Walk over the void with Void Walker")), 
			MaterialId.DIAMOND_BOOTS, Arrays.asList(new ESTrigger("boots", new ImpossibleTrigger())), new Rewards().setExperience(60), Frame.TASK),
	DETERMINED_CHEATER(THESE_BOOTS_WERE_MADE_FOR_WALKING, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/determined_cheater"), 
			Arrays.asList(new ESLocalization(Language.US, "Determined Cheater", "Break obsidian made using Void Walker")), MaterialId.OBSIDIAN,
			Arrays.asList(new ESTrigger("trident", new ImpossibleTrigger())), new Rewards().setExperience(100), Frame.GOAL),
	I_AINT_AFRAID_OF_NO_GHOSTS(DETERMINED_CHEATER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/i_aint_afraid_of_no_ghosts"), 
			Arrays.asList(new ESLocalization(Language.US, "I Ain't Afraid of No Ghosts", "Sleep 10 times while wearing an Unrest helmet")), MaterialId.WHITE_BED,
			Arrays.asList(new ESTrigger("trident", new ImpossibleTrigger(), 10)), new Rewards().setExperience(150), Frame.GOAL),
	IM_YOU_BUT_SHORTER(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/im_you_but_shorter"), 
			Arrays.asList(new ESLocalization(Language.US, "I'm You, But Shorter", "Teleport away from an enderman using Warp")), MaterialId.ENDER_PEARL,
	...
	PANZER_SOLDIER(IM_YOU_BUT_SHORTER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/panzer_soldier"), 
			Arrays.asList(new ESLocalization(Language.US, "Panzer Soldier", "Wear 4 pieces of armor with the highest level of Tank")), MaterialId.OBSIDIAN,
	...
	TOO_CLOSE(PANZER_SOLDIER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/too_close"), 
			Arrays.asList(new ESLocalization(Language.US, "Too Close", "Fail to activate Icarus in flight")), MaterialId.ELYTRA,
	...
	EXTRA_POWER(TOO_CLOSE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/extra_power"), 
			...
			MaterialId.ENCHANTED_GOLDEN_APPLE, Arrays.asList(new ESTrigger("power", new ImpossibleTrigger())), new Rewards().setExperience(150), Frame.GOAL),
	CRUISING_ALTITUDE(EXTRA_POWER, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/cruising_altitude"), 
			Arrays.asList(new ESLocalization(Language.US, "Cruising Altitude", "Travel to 12000m using Frequent Flyer")), MaterialId.ELYTRA,
	...
	DANGER_DEFEATED(CRUISING_ALTITUDE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/danger_defeated"), 
			Arrays.asList(new ESLocalization(Language.US, "Danger Defeated", "Kill a Wither using Gung Ho")), MaterialId.NETHER_STAR,
	...
	DIVINE_RETRIBUTION(DANGER_DEFEATED, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/divine_retribution"), 
			...
			MaterialId.TOTEM_OF_UNDYING, Arrays.asList(new ESTrigger("retribution", new ImpossibleTrigger())), new Rewards().setExperience(1000), Frame.CHALLENGE),
	LOOK_WHAT_YOU_MADE_ME_DO(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/look_what_you_made_me_do"), 
			Arrays.asList(new ESLocalization(Language.US, "Look What You Made Me Do", "Kill a Pillager using a Pillage crossbow")), MaterialId.CROSSBOW,
	...
	JUST_DIE_ALREADY(LOOK_WHAT_YOU_MADE_ME_DO, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/just_die_already"), 
			Arrays.asList(new ESLocalization(Language.US, "Just DIE Already!", "Kill a Phantom in one shot using Stone Throw")), MaterialId.PHANTOM_SPAWN_EGG,
			Arrays.asList(new ESTrigger("soulbound", new ImpossibleTrigger())), new Rewards().setExperience(75), Frame.TASK, 4),
	UNDERKILL(JUST_DIE_ALREADY, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/underkill"), 
			Arrays.asList(new ESLocalization(Language.US, "Underkill", "Hit the Ender Dragon using Stone Throw")), MaterialId.DRAGON_EGG,
	...
	WHERE_DID_THAT_COME_FROM(UNDERKILL, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/where_did_that_come_from"), 
			Arrays.asList(new ESLocalization(Language.US, "Where Did That Come From?", "Kill a skeleton from 100 blocks away using Sniper")), MaterialId.ARROW,
	...
	THAT_FOOD_IS_FINE(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/that_food_is_fine"), 
			...
			MaterialId.POISONOUS_POTATO, Arrays.asList(new ESTrigger("food", new ImpossibleTrigger())), new Rewards().setExperience(25), Frame.TASK),
	DEFLECTION(THAT_FOOD_IS_FINE, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/deflection"), 
			Arrays.asList(new ESLocalization(Language.US, "Deflection", "Kill a mob with an arrow deflected with Hard Bounce")), MaterialId.SHIELD,
	...
	IRON_MAN(DEFLECTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/iron_man"), 
			Arrays.asList(new ESLocalization(Language.US, "Iron Man", "Live through a death blow because of Iron Defense")), MaterialId.IRON_BLOCK,
	...
	IRONT_YOU_GLAD(ENCHANTMENT_SOLUTION, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/iront_you_glad"), 
			...
			MaterialId.IRON_INGOT, Arrays.asList(new ESTrigger("iron", new ImpossibleTrigger())), new Rewards().setExperience(25), Frame.TASK),
	FAST_AND_FURIOUS(IRONT_YOU_GLAD, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/fast_and_furious"), 
			Arrays.asList(new ESLocalization(Language.US, "Fast and Furious", "Break multiple blocks at once using Height++ or Width++")), MaterialId.DIAMOND_PICKAXE,
	...
	HEY_IT_WORKS(FAST_AND_FURIOUS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/hey_it_works"), 
			Arrays.asList(new ESLocalization(Language.US, "Hey It Works!", "Break a Shulker Box using Telepathy")), MaterialId.PURPLE_SHULKER_BOX,
	...
	NO_PANIC(HEY_IT_WORKS, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/no_panic"), 
			Arrays.asList(new ESLocalization(Language.US, "No Panic", "Grab an item directly over lava using Telepathy")), MaterialId.LAVA_BUCKET,
	...
	OVER_9000(NO_PANIC, new NamespacedKey(EnchantmentSolution.getPlugin(), "enchantments/over_9000"), 
			Arrays.asList(new ESLocalization(Language.US, "Over 9000", "Collect 9001 extra items with Height++ or Width++")), MaterialId.DIAMOND_PICKAXE,
	...
	ESAdvancement(NamespacedKey key, List<ESLocalization> defaultLocalizations, MaterialId icon, List<ESTrigger> triggers, Rewards rewards, Frame frame){
		this.namespace = key;
		this.icon = icon;
	...
	ESAdvancement(ESAdvancement parent, NamespacedKey key, List<ESLocalization> defaultLocalizations, MaterialId icon, List<ESTrigger> triggers, 
	...
	public void setIcon(MaterialId icon) {
		this.icon = icon;
	}
